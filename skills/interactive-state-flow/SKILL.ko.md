...
1. 사용자 의도를 지체 없이 기록한다.
2. 기준 상태를 source owner를 통해 커밋한다.
3. 후속 작업을 긴급도, 비용, 가시성, freshness 위험으로 분류한다.
4. 상호작용 경로를 가볍게 유지한다.
5. 비용이 큰 작업을 지연, 취소, 배치, 우선순위화, 가상화, 생략, 캐시하거나 정당한 경우에만 실행 경로를 옮긴다.
6. 비동기 또는 표현 결과는 여전히 fresh하고 유용할 때만 owner를 통해 커밋한다.
7. 스케줄러 내부가 아니라 동작 계약을 테스트한다.
